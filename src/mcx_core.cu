            ...
            switch (gcfg->srctype) {
                ...
                case (MCX_SRC_ARCSINE): {  // uniform distribution in zenith angle, arcsine distribution if projected in orthogonal plane
                    ...
                    if (gcfg->srctype == MCX_SRC_CONE) { // a solid-angle section of a uniform sphere
                        ...
                        temp_index=(int)(floorf(ang*launchsrc->param1.x)); // random a point on the CDF curve
                        ang=srcpattern[temp_index]+(srcpattern[temp_index+1]-srcpattern[temp_index])*(ang*launchsrc->param1.x-floorf(ang*launchsrc->param1.x)); // find the corresponding angle		          
                        // if: LED is rectangle
                        if(launchsrc->param1.y!=0.f){
                            // sphi and cphi here are to determine the launched azimuth angle
                            while(1){
                                // sampling launched position on circle LED panel
                                launch_x=-launchsrc->param1.y/2+launchsrc->param1.y*rand_uniform01(t); // assume LED panel center is at (0,0)
                                launch_y=-gcfg->src.param1.z/2+gcfg->src.param1.z*rand_uniform01(t);
                                // determine whether photon exceed the window. if exceed, resample the photon.        		          
                                if(gcfg->src.param2.x!=0.f){ // condition 1: radiated window is a rectangle.
                                    if(fabs(gcfg->src.param2.w*tanf(ang)*cphi+launch_x) <= gcfg->src.param2.x/2 && fabs(gcfg->src.param2.w*tanf(ang)*sphi+launch_y) <= gcfg->src.param2.y/2){
                                        break;
                                    }        		          
                                }else{ // condition 2: radiated window is a circle.
                                    if(powf(fabs(gcfg->src.param2.w*tanf(ang)*cphi+launch_x), 2.f) + powf(fabs(gcfg->src.param2.w*tanf(ang)*sphi+launch_y), 2.f) <= powf(gcfg->src.param2.z, 2.f)){
                                        break;
                                    }
                                }
                        ...
                        // else: LED is circular
                            ...
                            while(1){
                                ...
                                cir_r=gcfg->src.param1.w*sqrtf(rand_uniform01(t));
                        ...
                        // update legal p		          
                        p->x += gcfg->src.param2.w*tanf(ang)*cphi+launch_x;
                        p->y += gcfg->src.param2.w*tanf(ang)*sphi+launch_y;
                    } else {
